        ...
        <div className="card overflow-x-auto">
          <table className="w-full min-w-[760px] border-collapse">
            ...
            <tbody>
              {quotes.map((q) => (
                <tr key={q.id}>
                  <td className="td num">{q.quote_no}</td>
                  <td className="td text-ink-900">{q.project || '—'}</td>
                  <td className="td num">{q.quote_date}</td>
                  <td className="td">
                    <span className={`tag ${statusTagClass(q.status)}`}>{statusText(q.status)}</span>
                  </td>
                  <td className="td num font-semibold text-deep">
    ...
    <div className="space-y-4">
      <div className="card">
        <div className="flex flex-wrap items-center gap-3">
          ...
          <button type="button" className="btn ml-auto" onClick={() => navigate('/client')}>
            返回清單
          </button>
        </div>
                    ...
                    {sec.lines.map((dl) => {
                      ...
                      return (
                        <tr key={l.id}>
                          ...
                          <td className="td num">{money(lineAmount(orig, Number(l.qty)))}</td>
                          <td className="td">
                            <input
                              type="number"
                              min={0}
